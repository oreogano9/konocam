...
const checks = [
  ...
  {
    name: "native bridge records filtered videos to local gallery and Photos album",
    source: nativeSource,
    patterns: [
      /CAPPluginMethod\(name: "startVideoRecording"[\s\S]*CAPPluginMethod\(name: "stopVideoRecording"/,
      /private struct NativeVideoCapture[\s\S]*let lutData: Data[\s\S]*let filter: JSObject[\s\S]*let effects: JSObject/,
      /@objc func startVideoRecording[\s\S]*NativeVideoCapture\([\s\S]*lutData: lutData[\s\S]*filter: filter[\s\S]*effects: effects/,
      /@objc func stopVideoRecording[\s\S]*self\.processVideoCapture\(request: request, frames: frames, frameAttempts: frameAttempts, focusResetMode: focusResetMode, call: call\)/,
      /private func collectVideoRecordingFrames\(\)[\s\S]*latestNormalizedVideoFrameData\(request: request, after: videoCaptureLastFrameAt\)[\s\S]*videoCaptureFrames\.append\(frame\.data\)/,
      /private func processVideoCapture\(request: NativeVideoCapture/,
      /processPhotoData\([\s\S]*frameData,[\s\S]*lutData: request\.lutData,[\s\S]*intensity: request\.intensity,[\s\S]*effects: request\.effects/,
      /renderNativeStack\([\s\S]*photoData: lutOutput/,
      /"videoFiltered": true/,
      /saveVideoFile\(outputURL/,
